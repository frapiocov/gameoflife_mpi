...
## Conclusioni
In termini di scalabilità debole, il programma ha ottenuto le performance attese, la computazione sfrutta molto le risorse in termini di CPU poiché vengono analizzate indipendentemente tutte le celle della matrice e per ogni cella tutte le 8 celle nell'intorno. L'aumento della taglia della matrice fa diminuire di conseguenza le performance.

Per quanto riguarda la scalabilità forte, i risultati ottenuti sono inizialmente ottimi, i tempi diminuiscono, quasi dimezzano e lo speedup tende a quello ideale. Dai nove processori in poi i tempi si stabilizzano e lo speedup migliora meno velocemente. L'utilizzo della computazione parallela si dimostra altamente efficace per la risoluzione di questo tipo di problema.